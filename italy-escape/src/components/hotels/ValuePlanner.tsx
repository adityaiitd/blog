          ...
          <article key={stay.id} className="grid overflow-hidden border border-[var(--line)] lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className="p-6 md:p-9">
              <div className="mb-7 flex rounded-full border border-[var(--line)] p-1">
                {(["luxury", "value"] as TripTier[]).map((tier) => <button key={tier} onClick={() => dispatch({ type: "set-region-tier", stayId: stay.id, tier })} className={`flex flex-1 items-center justify-center gap-2 rounded-full px-4 py-3 text-xs uppercase tracking-wider ${stay.tier === tier ? "bg-[var(--ink)] text-[var(--paper)]" : ""}`}>{tier === "luxury" ? <Sparkles size={14} /> : <WalletCards size={14} />}{tier}</button>)}
              </div>
