...
import { SaveBar } from "@/components/share/SaveBar";
...
export function Nav() {
  ...
  const { undo, redo, canUndo, canRedo } = useTrip();
  return (
    <header className="no-print sticky top-0 z-[1000] border-b border-[var(--line)] bg-[color:var(--paper)/.92] backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-3 px-4 md:px-8">
        ...
        <div className="flex items-center gap-1">
          <div className="hidden md:block"><SaveBar /></div>
      ...
      <div className="border-t border-[var(--line)] px-3 py-2 md:hidden"><SaveBar /></div>
    </header>
  );
}
