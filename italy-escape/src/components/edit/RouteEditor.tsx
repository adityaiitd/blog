      ...
      <div className="border-t border-[var(--line)]">
        {state.routeLegs.map((leg, index) => (
          <div key={leg.id} className="grid gap-3 border-b border-[var(--line)] py-5 md:grid-cols-[2rem_1fr_1fr_10rem_1.4fr_7rem_3rem] md:items-center">
            ...
            <select aria-label={`Origin for route ${index + 1}`} value={leg.fromId} onChange={(e) => dispatch({ type: "update-route", id: leg.id, patch: { fromId: e.target.value } })} className="rounded-md bg-transparent p-2 text-sm">{state.destinations.filter((place) => !place.waypoint).map((place) => <option key={place.id} value={place.id}>{place.shortName}</option>)}</select>
            <select aria-label={`Destination for route ${index + 1}`} value={leg.toId} onChange={(e) => dispatch({ type: "update-route", id: leg.id, patch: { toId: e.target.value } })} className="rounded-md bg-transparent p-2 text-sm">{state.destinations.filter((place) => !place.waypoint).map((place) => <option key={place.id} value={place.id}>{place.shortName}</option>)}</select>
