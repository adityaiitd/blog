...
import { calculateCosts, hotelsTotal, money } from "@/lib/costCalculator";
...
export default function PrintPage() {
  ...
  return (
    <main className="page-shell max-w-5xl">
      ...
      <section className="print-break pt-8"><h2 className="mb-5 font-serif text-4xl">Hotels & budget</h2>{state.stays.map((stay) => { const hotelIds = [...new Set(stay.nightHotelIds?.length ? stay.nightHotelIds : [stay.hotelId])]; const names = hotelIds.map((id) => state.hotels.find((item) => item.id === id)?.name).filter(Boolean); return <div key={stay.id} className="flex justify-between border-t border-[var(--line)] py-4"><span>{stay.region} · {names.join(" → ")} · {stay.nights} nights</span><strong>{money(hotelsTotal(state.hotels, [stay]))}</strong></div>; })}<div className="mt-8 flex justify-between border-t-2 border-[var(--ink)] py-4 font-serif text-3xl"><span>Recommended trip budget</span><strong>{money(total.recommended)}</strong></div></section>
    </main>
  );
}
