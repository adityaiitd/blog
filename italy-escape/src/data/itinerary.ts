...
const purposeFor = (title: string, extras: Partial<Activity>): Activity["purpose"] =>
  extras.boatId ? "sail" : /flight|transfer|train|car to/i.test(title) ? "travel" : /pool|spa|resort|hotel morning/i.test(title) ? "relax" : /evening|lunch|caruso/i.test(title) ? "eat" : /morning|guided|touring|florence|san pantaleo|amalfi/i.test(title) ? "walk" : "see";
const rationaleFor = (title: string, purpose: Activity["purpose"]) => {
  if (purpose === "sail") return "The coast is most beautiful from the water; swimming access and island scale cannot be replicated by car.";
  if (purpose === "travel") return "This is the cleanest connection with enough margin to keep the day calm.";
  if (purpose === "relax") return "Protects the pace of the trip and makes the hotel spend earn its place.";
  if (purpose === "eat") return "Pairs a memorable setting with an otherwise unstructured part of the day.";
  if (/Florence/i.test(title)) return "One focused guide day gives context without turning Tuscany into a checklist.";
  if (/Val d’Orcia/i.test(title)) return "This is the landscape chapter: Siena, hill towns and wine country in one coherent loop.";
  return "Chosen for a strong sense of place while keeping enough room to wander.";
};
const a = (id: string, title: string, extras: Partial<Activity> = {}): Activity => {
  const purpose = purposeFor(title, extras);
  return {
    id, title, cost: 0, booked: false, weatherDependent: false, purpose,
    rationale: rationaleFor(title, purpose),
...
export const itinerary: ItineraryDay[] = [
  ...
  { id: "day-3", offset: 3, title: "San Pantaleo", summary: "Village morning, then an unhurried resort afternoon", locationId: "cala-di-volpe", notes: "The calm day before two consecutive boat days.", weatherBuffer: false, booked: false, activities: [a("a-4a", "San Pantaleo morning"), a("a-4b", "Pool and spa")] },
  { id: "day-4", offset: 4, title: "La Maddalena by sea", summary: "Caprera, Cala Coticcio, Budelli, Spargi and La Maddalena", locationId: "cala-di-volpe", notes: "Boat days are grouped at the tail of Sardinia to make a split stay practical.", weatherBuffer: false, booked: false, activities: [a("a-3", "Private boat: La Maddalena & Caprera", { weatherDependent: true, boatId: "boat-sardinia-1" })] },
  ...
  { id: "day-12", offset: 12, title: "The Amalfi Coast by sea", summary: "Conca dei Marini, Furore, Praiano, Positano and Nerano", locationId: "amalfi", notes: "Grouped at the start of Amalfi so later nights fully use the resort.", weatherBuffer: false, booked: false, activities: [a("a-14", "Private boat: Positano & Nerano", { weatherDependent: true, boatId: "boat-amalfi-2" })] },
  ...
  { id: "day-14", offset: 14, title: "Amalfi & Ravello", summary: "Amalfi morning, Ravello afternoon, drinks at Caruso", locationId: "amalfi", notes: "A final land day with a beautiful sunset finish.", weatherBuffer: false, booked: false, activities: [a("a-12a", "Amalfi morning"), a("a-12b", "Ravello and Caruso", { time: "Afternoon" })] },
